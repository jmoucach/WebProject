...
function pathfind() {
    if (openList.length > 0) {
        ...
        shuffle(current.neighbours, true);
        let neighbours = current.neighbours;
        for (let i = 0; i < neighbours.length; i++) {
            let neighbour = neighbours[i];
            if (neighbour.isWalkable && !closedList.includes(neighbour) && !openList.includes(neighbour)) {
    ...
    this.show = function(col) {
        if (this.isWalkable == false) {
            ...
            if (this.previous != undefined) {
                stroke(255);
			strokeWeight(4);
                line(this.i * w + w / 2, this.j * h + h / 2, this.previous.i * w + w / 2, this.previous.j * h + h / 2);
            }
        }
    }
