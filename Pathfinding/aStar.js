...
function heuristic(cell, goal) {
    // return (Math.abs(cell.i - goal.i) + Math.abs(cell.j - goal.j)); // Manhattan heuristic
    return (Math.floor(Math.sqrt((cell.i - goal.i) * (cell.i - goal.i) + (cell.j - goal.j) * (cell.j - goal.j)))); // Eculidian heuristic
}
    ...
    if (openList.length > 0) {
        ...
        for (let i = 0; i < neighbours.length; i++) {
            ...
            if (neighbour.isWalkable && !closedList.includes(neighbour)) {
                ...
                if (newPath) {
                    neighbour.hCost = heuristic(neighbour, endPosition)
...
function Spot(i, j) {
    ...
    this.show = function(col) {
        if (this.isWalkable == false) {
            ...
            fill(255);
            textSize(8);
            text(this.gCost, this.i * w + w / 4, this.j * h + h / 4 + 5);
            text(this.hCost, this.i * w + w * 3 / 4, this.j * h + h / 4 + 5);
            textSize(10);
            text(this.fCost, this.i * w + w / 2, this.j * h + h * 3 / 4 + 5);
        }
    }
